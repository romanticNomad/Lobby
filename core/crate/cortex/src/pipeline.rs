...
//         let tx_hash = if let BroadcastOutcome::Submitted { txn_hash } = outcome {
//             txn_hash
//         }

//         match outcome {
//             BroadcastOutcome::Submitted { txn_hash } => {
//                 ctx.status.set(execution_id, PipelineStatus::Broadcasted { tx_hash: format!("{txn_hash:#x}") });
//                 return;
//             }
//             BroadcastOutcome::Rejected { reason } => {
//                 // hard failiur
//                 release_nonce(&nonce_handle, execution_id, &ctx.retry_config).await;
//                 let err = CortexError::Internal(reason);
//                 record_faliure(&ctx.status, execution_id, &err);
//                 return;
//             }
//             BroadcastOutcome::Unexpected { source } => {
//                 // hard faliur
//                 let err = CortexError::Internal(source);
//                 record_faliure(&ctx.status, execution_id, &err);
//                 return;
//             }
//         };
